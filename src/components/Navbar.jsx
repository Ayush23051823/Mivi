...
const Navbar = ({ user, logout }) => {   // ⬅️ accept user & logout as props
  ...
  const handleLogout = async () => {
    try {
      if (logout) {
        const result = await logout(); // logout can return { message } or nothing
        const message = result?.message || "Logged out successfully";
        toast.success(message);
      } else {
        console.warn("logout function not provided to Navbar");
      }
    } catch (err) {
      console.error(err);
    ...
    <nav className="bg-black text-gray-200 flex justify-between items-center p-4 h-20 text-sm md:text-[15px] font-medium text-nowrap">
      ...
      <div className="flex items-center space-x-4 relative">
        ...
        <Link to={user ? "/ai-recommendations" : "/signin"}>
          <button className="bg-[#e50914] px-5 py-2 text-white cursor-pointer hover:bg-[#BB3B3B]">
            Get AI Movie Picks
          </button>
        </Link>
        ...
        {!user ? (
          <Link to={"/signin"}>
            <button className="border border-[#333333] py-2 px-4 cursor-pointer hover:bg-white hover:text-black">
              Sign In
            </button>
          </Link>
        ) : (
